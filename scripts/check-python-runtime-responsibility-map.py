...
function occurrenceGroups(items) {
  const result = grouped(
    items,
    (item) => `${item.path}\0${item.owner}\0${item.symbol}`,
  );
...
function qualifiedOccurrenceMatches(current, baseline) {
  ...
  const unmatched = () => {
    const matchedBaseline = new Set(
      [...matches.values()].map(({ id }) => id),
    );
    return {
      current: current.filter((item) => !matches.has(item.id)),
      baseline: baseline.filter((item) => !matchedBaseline.has(item.id)),
    };
  };
  uniqueGlobalMatches(
    current,
...
function qualifiedTransition(current, baseline) {
  const qualified = qualifiedOccurrenceMatches(current, baseline);
  const matchedBaseline = new Set();
  const functions = current.map((item) => {
    const previous = qualified.matches.get(item.id) || null;
...
function measure(files, paths) {
  ...
  const transition = qualifiedTransition(
    snapshot.functions,
    riskBaseline.functions,
  );
  const selectedPaths = new Set(paths);
  const ambiguous = transition.findings.filter(({ paths: findingPaths }) =>
...
def function_risk(
    ...
    if result.returncode != 0:
        raise AssertionError(
            result.stderr.strip()
            or f"function-risk measurement failed with exit {result.returncode}"
        )
...
def main() -> None:
    ...
    assert responsibility_map["functionRiskAlgorithmVersion"] == (
        "checker-qualified-occurrence-v2"
    )
